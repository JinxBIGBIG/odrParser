...
double DataOperation::dist(POINT p1,POINT p2)                // 返回两点之间欧氏距离
{
    return( sqrt( (p1.x-p2.x)*(p1.x-p2.x)+(p1.y-p2.y)*(p1.y-p2.y) ) );
}
bool DataOperation::equal_point(POINT p1,POINT p2)           // 判断两个点是否重合
{
...
r<0：ep在矢量opsp的顺时针方向
*******************************************************************************/
double DataOperation::multiply(POINT sp,POINT ep,POINT op)
{
...
r>0：两矢量夹角为钝角
*******************************************************************************/
double DataOperation::dotmultiply(POINT p1,POINT p2,POINT p0)
{
...
*******************************************************************************/
bool DataOperation::online(LINESEG l,POINT p)
{
    return( (multiply(l.e,p,l.s)==0) &&( ( (p.x-l.s.x)*(p.x-l.e.x)<=0 )&&( (p.y-l.s.y)*(p.y-l.e.y)<=0 ) ) );
}
// 返回点p以点o为圆心逆时针旋转alpha(单位：弧度)后所在的位置
POINT DataOperation::rotate(POINT o,double alpha,POINT p)
{
    POINT tp;
    p.x-=o.x;
    p.y-=o.y;
...
原理：
	...
	-1<r<1 && r'<=0	angle = -arccos(r)
*/
double DataOperation::angle(POINT o,POINT s,POINT e)
{
...
*/
double DataOperation::relation(POINT p,LINESEG l)
{
    LINESEG tl;
    tl.s=l.s;
    tl.e=p;
...
// 求点C到线段AB所在直线的垂足 P
POINT DataOperation::perpendicular(POINT p,LINESEG l)
{
...
double DataOperation::ptolinesegdist(POINT p,LINESEG l,POINT &np)
{
    double r=relation(p,l);
    if(r<0)
    {
...
double DataOperation::ptoldist(POINT p,LINESEG l)
{
    return abs(multiply(p,l.e,l.s))/dist(l.s,l.e);
}
/* 计算点到折线集的最近距离,并返回最近点.
注意：调用的是ptolineseg()函数 */
double DataOperation::ptopointset(int vcount,POINT pointset[],POINT p,POINT &q)
{
    int i;
...
bool DataOperation::CircleInsidePolygon(int vcount,POINT center,double radius,POINT polygon[])
{
    POINT q;
    double d;
    q.x=0;
...
bool DataOperation::intersect(LINESEG u,LINESEG v)
{
...
bool DataOperation::intersect_A(LINESEG u,LINESEG v)
{
...
bool DataOperation::intersect_l(LINESEG u,LINESEG v)
{
    return multiply(u.s,v.e,v.s)*multiply(v.e,u.e,v.s)>=0;
}
// 根据已知两点坐标，求过这两点的直线解析方程： a*x+b*y+c = 0  (a >= 0)
LINE DataOperation::makeline(POINT p1,POINT p2)
{
    LINE tl;
    int sign = 1;
    tl.a=p2.y-p1.y;
...
double DataOperation::slope(LINE l)
{
    if(abs(l.a) < 1e-20)
        return 0;
    if(abs(l.b) < 1e-20)
...
// 返回直线的倾斜角alpha ( 0 - pi)
double DataOperation::alpha(LINE l)
{
    if(abs(l.a)< EP)
        return 0;
    if(abs(l.b)< EP)
...
// 求点p关于直线l的对称点
POINT DataOperation::symmetry(LINE l,POINT p)
{
    POINT tp;
...
bool DataOperation::lineintersect(LINE l1,LINE l2,POINT &p) // 是 L1，L2
{
...
bool DataOperation::intersection(LINESEG l1,LINESEG l2,POINT &inter)
{
    LINE ll1,ll2;
...
本程序默认第一个条件已经满足
*/
bool DataOperation::issimple(int vcount,POINT polygon[])
{
    int i,cn;
    LINESEG l1,l2;
    for(i=0;i<vcount;i++)
...
void DataOperation::checkconvex(int vcount,POINT polygon[],bool bc[])
{
    int i,index=0;
    POINT tp=polygon[0];
...
bool DataOperation::isconvex(int vcount,POINT polygon[])
{
    bool bc[MAXV];
...
double DataOperation::area_of_polygon(int vcount,POINT polygon[])
{
    int i;
    double s;
    if (vcount<3)
...
// 如果输入顶点按逆时针排列，返回true
bool DataOperation::isconterclock(int vcount,POINT polygon[])
{
    return area_of_polygon(vcount,polygon)>0;
}
// 另一种判断多边形顶点排列方向的方法
bool DataOperation::isccwize(int vcount,POINT polygon[])
{
    int i,index;
    POINT a,b,v;
    v=polygon[0];
...
射线法判断点q与多边形polygon的位置关系，要求polygon为简单多边形，顶点逆时针排列
   ...
   如果点在多边形外：	返回2
*********************************************************************************************/
int DataOperation::insidepolygon(int vcount,POINT Polygon[],POINT q)
{
    int c=0,i,n;
    LINESEG l1,l2;
...
bool DataOperation::InsideConvexPolygon(int vcount,POINT polygon[],POINT q) // 可用于三角形！
{
    POINT p;
    LINESEG l;
    int i;
...
len为输出的凸包上的点的个数
**********************************************/
void DataOperation::Graham_scan(POINT PointSet[],POINT ch[],int n,int &len)
{
    int i,j,k=0,top=2;
    POINT tmp;
...
void DataOperation::ConvexClosure(POINT PointSet[],POINT ch[],int n,int &len)
{
...
**********************************************************************************************/
bool DataOperation::LinesegInsidePolygon(int vcount,POINT polygon[],LINESEG l)
{
    // 判断线端l的端点是否不都在多边形内
...
但要注意函数的声明和调用要做相应变化
**********************************************************************************************/
void DataOperation::AddPosPart(double x, double y, double w, double &xtr, double &ytr, double &wtr)
{
...
void DataOperation::AddNegPart(double x, double y, double w, double &xtl, double &ytl, double &wtl)
{
    if ( abs(wtl + w)<1e-10 )
...
void DataOperation::AddRegion ( double x1, double y1, double x2, double y2, double &xtr, double &ytr,
...
POINT DataOperation::cg_simple(int vcount,POINT polygon[])
{
...
POINT DataOperation::gravitycenter(int vcount,POINT polygon[])
{
    POINT tp;
...
结论	： x坐标最大，最小的点肯定是凸顶点
	y坐标最大，最小的点肯定是凸顶点
************************************************/
POINT DataOperation::a_point_insidepoly(int vcount,POINT polygon[])
{
    POINT v,a,b,r;
    int i,index;
    v=polygon[0];
...
/***********************************************************************************************/
void DataOperation::pointtangentpoly(int vcount,POINT polygon[],POINT p,POINT &rp,POINT &lp)
{
    LINESEG ep,en;
    bool blp,bln;
    rp=polygon[0];
...
bool DataOperation::core_exist(int vcount,POINT polygon[],POINT &p)
{
    int i,j,k;
    LINESEG l;
    LINE lineset[MAXV];
...
用途	： 因为圆为凸集，所以判断点集，折线，多边形是否在圆内时，
	只需要逐一判断点是否在圆内即可。
*******************************************************************************/
bool DataOperation::point_in_circle(POINT o,double r,POINT p)
{
...
*******************************************************************************/
bool DataOperation::cocircle(POINT p1,POINT p2,POINT p3,POINT &q,double &r)
{
...
int DataOperation::line_circle(LINE l,POINT o,double r,POINT &p1,POINT &p2)
{
    return true;
}
...
POINT DataOperation::rect4th(POINT a,POINT b,POINT c)
{
    POINT d;
...
内含： return 5；
*/
int DataOperation::CircleRelation(POINT p1, double r1, POINT p2, double r2)
{
...
// 调用ptoldist函数，在第4页
bool DataOperation::CircleRecRelation(POINT pc, double r, POINT pr1, POINT pr2, POINT pr3, POINT pr4)
{
...
double DataOperation::P2planeDist(double x, double y, double z, double a, double b, double c, double d)
{
    return fabs(a*x+b*y+c*z+d) / sqrt(a*a+b*b+c*c);
}
//点是否在直线同侧：
//两个点是否在直线同侧，是则返回true
bool DataOperation::SameSide(POINT p1, POINT p2, LINE line)
{
...
void DataOperation::reflect(double a1,double b1,double c1,double a2,double b2,double c2,double &a,double &b,double &c)
{
    double n,m;
    double tpb,tpa;
    tpb=b1*b2+a1*a2;
...
//矩形包含：
// 矩形2（C，D）是否在1（A，B）内
bool DataOperation::r2inr1(double A,double B,double C,double D)
{
    double X,Y,L,K,DMax;
    if (A < B)
    {
...
//两圆交点：
// 两圆已经相交（相切）
void  DataOperation::c2point(POINT p1,double r1,POINT p2,double r2,POINT &rp1,POINT &rp2)
{
    double a,b,r;
    a=p2.x-p1.x;
    b=p2.y-p1.y;
...
//两圆公共面积：
// 必须保证相交
double DataOperation::c2area(POINT p1,double r1,POINT p2,double r2)
{
    POINT rp1,rp2;
...
//圆和直线关系：
//0----相离 1----相切 2----相交
int DataOperation::clpoint(POINT p,double r,double a,double b,double c,POINT &rp1,POINT &rp2)
{
    int res=0;
...
//内切圆：
void DataOperation::incircle(POINT p1,POINT p2,POINT p3,POINT &rp,double &r)
{
...
void DataOperation::cutpoint(POINT p,double r,POINT sp,POINT &rp1,POINT &rp2)
{
    POINT p2;
    p2.x=(p.x+sp.x)/2;
    p2.y=(p.y+sp.y)/2;
...
返回	–1 ： 左旋；
*/
int DataOperation::rotat(LINESEG l1,LINESEG l2)
{
